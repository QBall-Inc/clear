...
exports.KnowledgeDatabase = void 0;
exports.exportJsonIndex = exportJsonIndex;
exports.importJsonIndex = importJsonIndex;
const better_sqlite3_1 = __importDefault(require("better-sqlite3"));
const path = __importStar(require("path"));
const fs = __importStar(require("fs"));
const KNOWLEDGE_TYPES = ['technical-decision', 'business-rule', 'architectural-pattern', 'lesson-learned'];
const KNOWLEDGE_STATUSES = ['active', 'superseded', 'deprecated', 'archived'];
function isKnowledgeType(v) {
    return KNOWLEDGE_TYPES.includes(v);
}
function isKnowledgeStatus(v) {
    return KNOWLEDGE_STATUSES.includes(v);
}
...
/**
 * SQL statements for schema creation (v3 - includes deprecation fields)
 */
const SCHEMA_SQL = `
-- Knowledge entries table (v4 with unified supersession)
...
INSERT OR REPLACE INTO index_metadata (key, value) VALUES ('schema_version', '${SCHEMA_VERSION}');
`;
// Note: Migrations are handled programmatically in migrateToV2() and migrateToV3()
// to support idempotent column additions (SQLite ALTER TABLE limitations)
/**
...
class KnowledgeDatabase {
    ...
    initialize() {
        try {
            ...
            // Log warning so failure isn't silent, then return false for caller to handle fallback
            const message = error instanceof Error ? error.message : 'Unknown error';
            console.warn(`[knowledge-db] Database initialization failed: ${message}. Falling back to JSON storage.`);
            return false;
        }
    }
    /**
    ...
    upsertEntry(entry) {
        ...
        INSERT OR REPLACE INTO knowledge_entries (
          ...
          archived_at, deprecation_type, superseded_at
        ) VALUES (
          @id, @type, @title, @status, @tags, @created, @created_session,
          @modified, @supersedes, @superseded_by, @description, @file_path, @tfidf_vector,
          @workpackage_id, @phase_id, @deprecated_at, @deprecated_reason,
    ...
    rowToEntry(row) {
        return {
            id: row.id,
            type: isKnowledgeType(row.type) ? row.type : 'lesson-learned',
            title: row.title,
            status: isKnowledgeStatus(row.status) ? row.status : DEFAULT_STATUS,
